...
/**
 * The phone layout: one full-screen card per swipe, with the waiting state or the
 * end card as the final page. Desktop uses the Reader instead.
 */
export function SwipeFeed({ feed, field, mode }: { feed: ReturnType<typeof useFeed>; field: string; mode: FeedMode }) {
  const [height, setHeight] = useState(0);
  const fieldName = feed.page?.field.name ?? field;
    ...
    <View style={styles.fill} onLayout={onLayout}>
      ...
      {height > 0 && feed.page && (
        <FlatList
          ...
          renderItem={({ item }) =>
            item.kind === "card" ? (
              <Card card={item.card} fieldName={fieldName} variant="swipe" height={height} onToggleSave={feed.toggleSave} />
            ) : (
              footer
            )
          }
