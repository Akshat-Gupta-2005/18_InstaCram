...
│   └── fact-check-confirm.md          one claim + one excerpt: is it false about the SAME subject? OFF in production — measured 5/8 on planted lies (FACT_CHECK_CONFIRM)
├── services/
...
│   │       ├── expansion/             queue.ts (the field_expansion job table) · worker.ts (claim → candidates → resolve; retry, reaper)
│   │       ├── topics/                candidates.ts (the LLM agent) · prompts.ts (loads prompts/*.md) · embeddingText.ts (the ONE place the embedded string is built) · resolve.ts (candidate → reuse/join/requeue/create, under a name lock)
│   │       ├── vectors/               collections.ts (the two Qdrant collections, create-if-missing) · ensure.ts (`npm run vectors:ensure`) · topicIndex.ts (search only; 3.4) · verifyTopicIndex.ts (`npm run vectors:verify`, real stack)
│   │       ├── calibration/           fields.ts (8 tagged fields) · generate.ts (phase A) · sweep.ts (the threshold measurement) · explainResolution.ts (why a field's candidates did or did not reuse — real scores, P36)
│   │       └── db/
...
│   │   ├── app/workers/topics.py      claims pending topics → runs the graph; the claim window is also degraded-run backoff (5.4)
...
│   │   ├── scripts/recheck_live.py    re-runs the fixed gate over live cards the blind one passed; dry-run unless --retire (P33)
...
├── clients/app/                       ONE Expo codebase (SDK 57, Expo Router) for website + iOS + Android; web dev server on 8082 (8081 is embeddings)
│   ├── app.json, package.json         scheme instacram, web.output "single", `npm run web|android|ios`, `npm run typecheck`
│   ├── AGENTS.md                      read the versioned Expo docs before writing code — the SDK moves
│   ├── src/app/                       routes: _layout (fonts + session gate) · login · (app)/index · (app)/feed/[field] · (app)/saved
│   ├── src/api/                       client.ts (the one request function; `keepalive` for sends during unload) · types.ts, mirroring API-CONTRACT.md
│   ├── src/auth/                      session.tsx + token storage: keychain on native, localStorage on web (.web.ts)
│   ├── src/feed/use-feed.ts           paging, polling, view logging — the contract's two client rules live here (P38)
│   ├── src/feed/reader.tsx            desktop reader: index-based, peeking neighbours, J/K/Space/S/Esc, field-wide card numbers
│   ├── src/feed/swipe-feed.tsx        phones: one full-screen card per swipe
│   ├── src/feed/card.tsx, rich-text.tsx, footer.tsx   the card, its typeset prose, and the waiting / end-of-field states
│   ├── src/ui/                        primitives + the desktop shell (sidebar, top bar, theme toggle)
...
└── Docs/
    ...
    ├── uml/                           24 Mermaid diagrams: HLD (context/container/deployment), LLD (components, classes), ER, sequences, state machines, decision flows
...
*Updated 2026-09-18 — the first four bullets here described W4/W5 gaps that are now closed; they are kept nowhere but the history.* Both halves are wired: a field request enqueues an expansion, a miss becomes a pending topic, the pipeline's workers generate and persist it, the outbox writes its vector, and the app reads all of it. What is missing is below.

- **No clients on a phone.** The Expo app runs as a website against the local API; iOS and Android builds have never been run on a device, and the local-storage mirror (2b.3) does not exist.
- **Auth is developer-only.** `AUTH_MODE=dev-login` checks one ID and password from `.env` and issues a signed token. There is no rate limiting, the web token sits in `localStorage`, and `AUTH_MODE=dev` (tests) still trusts any token as an identity. Google sign-in via Firebase (task 2b.1) waits on credentials. **This must be replaced before anything is network-reachable.**
- **No K8s manifests, no Prometheus scrape config, no Grafana dashboards, no E2E suite** (W7). `/metrics` exists on the pipeline; nothing collects it. Monitoring today is `/health` on both services, the counters in Postgres, and scripts run by hand.
- **W5's measurements are not taken.** Cache hit rate on a clean database, and time-to-first-card on a hit versus a miss — the two numbers W5 exists to produce (§6).
- **The developer database is not a clean population.** Hand-written seed rows and script-created topics break the same-producer symmetry the cache depends on (P36), so no hit rate may be read off it.
...
| 3b | `services/serving/src/expansion/worker.ts` | **built, live** | A first request only **enqueues** the field's expansion (224ms); a worker in serving runs candidates → steps 4–5 for each. |
| 4 | `services/serving/src/topics/resolve.ts` · `src/vectors/topicIndex.ts` | **built, live** | `embed(name + description)` is searched against the Topic-Name Vector DB at threshold `0.955`, **and** Postgres is read for same-name topics whose vector is still owed — under an advisory lock on the name, so simultaneous requests cannot both create it (P35). **The branch point.** |
| 5a | `services/serving/src/topics/resolve.ts` | **built, live** | **MATCH** → write one `Field_Topic` row; the topic's existing cards serve under the new field **immediately** — seen live: `Java Data Structures` showed 2 reused `TreeSet` cards at t+0, before anything was generated. A matched `empty` topic is **re-queued as the same row**, never duplicated (invariant 6, P16). |
| 5b | `resolve.ts` → `services/pipeline/app/workers/topics.py` | **built, live** | **NO MATCH** → create the topic as `pending` with its outbox row, in one transaction. The pending row **is** the pipeline's job: the topic worker claims it (`FOR UPDATE SKIP LOCKED`), runs steps 6–9, and the topic turns `ready`. Cards arrive **per topic** as each finishes — seen live, 2 → 5 cards at t+50s with `generating` true throughout. |
...
| Topic-Name Vector DB | `Done` (task 3.4) | Qdrant collection, `{ topic_id, embed(name + description) }`. The description is the candidate generator's one-liner, so query and index vectors come from the same producer — symmetric by construction. Resolves P9. Created at **768 dims / Cosine**, matching the calibrated model. **One writer, one reader:** the pipeline's outbox worker writes, keyed by topic id; serving only searches. **Verified against the real index** (`npm run vectors:verify`): all 14 topics retrieve themselves at 1.0000, an unrelated query peaks at 0.7923 (below 0.955), 0 orphan points. A hit is never trusted on its own — it is confirmed against Postgres before linking, since a vector can outlive its row. | `src/vectors/topicIndex.ts` · `src/vectors/collections.ts` | ★★ |
...
| Transactional outbox + worker | `Done` — **running since 2026-09-16** | `Topic` row and `Outbox` row commit in one Postgres transaction; a worker drains it into Qdrant with bounded retry, then dead-letters. **Proved by killing Qdrant mid-drain**: the row retried with its error recorded, the worker survived, and the vector appeared on restart. Three distinct failures are handled separately — Qdrant down (retry), poisoned row (dead-letter), and **worker death mid-row**; a reaper returns those to the queue. Resolves P11. **Correction (P37):** until 2026-09-16 the deployed service never started this worker — every proof above ran through scripts calling `drain_once` by hand, and 19 owed vectors sat untouched for hours. It now starts with the service, reconnects after losing the database, and reports its state in `/health`. First live pass: `claimed=19 written=19`, Qdrant 14 → 33. | `app/workers/outbox.py` · `app/main.py` | ★★ |
| Topic worker | `Done` (W5 5.4) | Claims pending topics and runs the generation graph, one at a time (the model is local; parallel runs would contend for one GPU). `status` stays `pending` while it runs, because the feed reads that as generating; only `claimed_at` changes. The claim's stale window is **both** dead-worker recovery **and** backoff for `degraded` runs, which deliberately leave a topic pending — without it a broken fact-checker is retried every five seconds. Every one of those was **seen failing** with its mechanism removed. **Live:** `Collectors` ready 3/0, `ArrayList` ready 3/1 (~81s), `LinkedList` ready 2/2 (~410s — a long source, chunked fact-checking). | `app/workers/topics.py` · `tests/test_topic_worker.py` | ★★ |
...
| App — web, iOS, Android (W6) | `Building` | **Reading-room design (2026-09-17):** parchment palette with an "ink" dark variant, Newsreader serif for reading, small-caps Inter labels, deep red accent. Desktop is a reader — sidebar, breadcrumb, progress rail, the previous and next cards peeking, a keyboard panel, and J/K/Space/S/Esc — verified in Chrome at 1440 px: J moves, Space reveals, a view is logged, no console errors. Phones keep one full-screen card per swipe in the same style. Built from real data only: the reference's citation percentage, focus meter, marginal notes and per-card diagrams were replaced by true equivalents or left out. One Expo codebase (SDK 57, Expo Router) in `clients/app`. Developer login, home, a feed of one full-screen card per swipe with all five MVP elements (source link, trust label, why it matters, tap-to-reveal recall, save), polling while a field generates, the end-of-field card with "more topics" and related fields, revision mode, and saved cards. **Verified in real Chrome:** login and wrong-password refusal, the feed card, reveal, save (`200`), and a view logged (`202`) only after the card stayed on screen — never on fetch, the contract's first rule. Not yet run on a phone. **Two feed bugs found in first real use (2026-09-17), both fixed and verified by a deterministic browser test that inserts a card mid-wait:** (1) a field whose cards loaded after the screen had rendered opened on a permanent spinner, cards scrolled out of sight above it, with the feed re-requested 5 times in 6 s; (2) a card that finished generating while the user sat on the waiting page was fetched but never came on screen — the browser re-snapped to the waiting page. Fix (2) was seen failing with its fix removed. The waiting page now also says that a new field's first cards can take several minutes. **Two view-log faults found and fixed 2026-09-18 (P38):** displayed cards were held in memory and sent every 5s or on unmount, so a refresh or closed tab lost them and the same card came back next visit — now sent ~1s after display and on `pagehide`/background with `keepalive`; and every feed screen ever opened stayed mounted, keyboard listener and all, logging cards nobody saw — now an unfocused feed logs nothing, polls nothing and ignores keys, and Home is reached with `dismissTo`. **Cards are numbered across the field** ("Card 64 of 91") from the feed's `progress`, re-based at each page load after that visit's views are sent. | `clients/app` | ★★ |
| Developer login | `Done` | `AUTH_MODE=dev-login`: ID + password from `.env`, compared in constant time, exchanged for an HMAC-signed token that every request must carry. A bare-name token — which the old dev mode accepted — now gets `401`. Seen failing with signature checking disabled. Google sign-in via Firebase slots into the same `requireAccount`. | `src/auth/` · `src/routes/auth.ts` · `tests/auth.test.ts` | ★ |
...
| Candidate Topic Generator | `Done` | LLM agent: field name → candidate topic names **+ a one-line description each**. Lives in the *serving* service, not the pipeline, because it runs on every request including pure cache hits. Measured: 156 candidates over 8 fields, 8/8 fields parsing cleanly once JSON mode landed; live through the expansion worker, 20 candidates in one 54.4s run. **Its descriptions are only comparable with its own** — topics described by any other producer are largely invisible to it (P36). | `src/topics/candidates.ts`, `src/topics/prompts.ts` | |
| Topic cache lookup (strict threshold) | `Built`, not yet wired | One candidate → **reused** (ready topic), **joined** (already pending), **requeued** (empty topic, same row), or **created** (new pending topic + outbox row). Biased toward regenerating a duplicate rather than risking a false merge. **Threshold `0.955` on `intfloat/e5-base-v2`** — the lowest value with zero false merges over 44 labelled pairs, keeping 67% reuse (§4). Two things a naive lookup gets wrong, both tested: **simultaneous requests** (8 at once create exactly one topic), and **outbox lag** — Qdrant cannot see a topic whose vector is not written yet, so the lookup also reads Postgres for same-name topics under the lock. Duplicates are tie-broken **deterministically** (score, then a topic with cards, then oldest), or two fields could link different copies and 5.5 would fail. The race test was proven able to fail — see P35. | `src/topics/resolve.ts` · `tests/resolve.test.ts` | ★★★ |
...
| View logging | `Done` | One `User_View` row per **displayed** card — never per fetched card, which prefetch makes a different number (P12). Built before it had a consumer because view history is the only data that cannot be backfilled; it since acquired one early, as the feed's paging mechanism. **The client half is where this gets lost or invented (P38):** a card counts after 600ms on screen, is sent ~1s later and again on `pagehide`/background with `keepalive` (a refresh runs no unmount), and only the focused screen may count anything at all. Both faults were silent and neither raised an error. | `src/routes/account.ts` · `src/repo/accounts.ts` · `clients/app/src/feed/use-feed.ts` | ★★ |
| Progress through a field | `Done` | `progress: { viewed, total }` on feed and revision pages, counted over the population a feed can serve — live cards of ready topics — so `viewed` can never exceed `total`. Lets the reader number cards across visits ("Card 64 of 91") instead of restarting at 1 each time. Seen failing with `scroll` in place of `live_scroll`: a retired card someone saw made it "of 6" instead of "of 5". | `src/repo/feed.ts` · `tests/feed.test.ts` | |
...
- [x] **Confirm the frontend frameworks.** Done 2026-09-17 — one Expo codebase for web, iOS and Android (`clients/app`); the earlier Next.js web client is dropped. W6 is started.
- [ ] **Seed placeholder cards are the first thing a user sees in some fields, labelled "Verified from source".** Found by driving the app in Chrome: *Java Data Structures* opens on "TreeSet: seeded card 1. Placeholder text standing in for a generated card." The field reuses the seeded TreeSet topic, and the feed orders oldest topics first. A dev-data problem, not an app bug — but it puts the invariant-4 label on text that verified nothing, in front of a user.
...
- [ ] **Topics created outside the candidate generator are near-invisible to the cache, and the dev database is full of them** (P36). `seed.ts`, `measure.py` and `run_full.py` all write hand-authored descriptions, breaking P9's same-producer symmetry; measured against generator candidates they score **0.87–0.93**, below `0.955`. The dev database now holds four `HashMap` rows and three `ArrayList` rows for this reason. Not a resolver bug — but any cache hit rate read off this database is contaminated. **Decided 2026-09-16:** the W5 hit-rate measurement runs on a **separate, empty database** where only the generator creates topics; the dev database is left untouched.
- [ ] **The fact-check gate has a measured over-strict failure mode on the chunked path, and the negative control cannot see it.** First live rejections through the fixed gate, read one by one: `ArrayList` "doubles its capacity" — **correctly** rejected (Java grows by 1.5×; the Javadoc promises no policy). `LinkedList` "pointers to next and previous, so you can move both ways" — **falsely** rejected: the checker's own reason says the claim is *supported*, then fails it because the excerpt also mentions extra memory per node, which the chunk prompt explicitly says is not grounds. `LinkedList` "doubly-linked lists are used in LISP" — right verdict (LISP uses *singly*-linked cons cells), wrong reason (an inference, not a contradiction). The negative control only ever measured leniency — does it catch planted lies — never over-strictness on a true card whose source says more. With no retry, a false rejection is a card lost for good. **Tried 2026-09-16:** a confirmation call before rejecting took true cards kept 2/5 → 5/5 but planted lies caught 8/8 → 5/8, and the 5/5 was inflated because its prompt's examples were the test cases. **Shipped off** (`FACT_CHECK_CONFIRM`); being reworked against held-out cases. Until then production accepts the over-strictness in exchange for catching lies.
...
- [x] **CI green.** Done — run 5 on `dd8878e` is the first passing run in the project's history, after 4 consecutive failures that went unread while CI was believed never to have run (P34). The `serving` job had no Postgres for tests that require one. **Keep reading the results:** CI already caught the editable-install bug on its very first run, a day before it was found by hand.
...
- [ ] **Replace dev auth before anything is exposed.** The running app now uses `AUTH_MODE=dev-login` (2026-09-17): a developer ID and password from `.env`, checked by the server, exchanged for a signed token — so a request can no longer name its own identity. Still not fit for a network: one shared account, no rate limiting, and the web build keeps its token in `localStorage`. `AUTH_MODE=dev`, which trusts any token, remains for the test suite only. Google sign-in through Firebase (task 2b.1) is the replacement.
