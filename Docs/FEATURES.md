...
- [ ] **Decide how the scraper finds the *right* article — a W4 blocker, now measured rather than suspected.** Across 10 topics, *TreeSet*, *ArrayList*, *ConcurrentSkipListMap* and *LinkedHashMap* all resolved to the same *Java collections framework* article, byte-identical at 22,789 chars, and *HashMap* resolved to *Hash table*. All 5 Behavioural Economics topics resolved well. This **contradicts the spike's own hypothesis** that non-technical sourcing would be the thin side: technical topics are the ones that mis-match, and four distinct cards would be generated from the same grounding text. Options to weigh: a more specific search query, exact-title lookup with search as fallback, section-level rather than whole-article extraction, or a second non-Wikipedia source. Note this is a *content-quality* problem — the retrieval works fine, which is precisely why a char-count metric missed it.
