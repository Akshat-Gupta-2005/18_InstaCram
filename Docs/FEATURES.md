...
| Topic cache lookup (strict threshold) | `Planned` | Searches `embed(name + description)` at a strict similarity threshold; `status = 'empty'` topics count as misses. Biased toward regenerating a duplicate rather than risking a false merge. **Threshold chosen and measured: `0.955` on `intfloat/e5-base-v2`** — the lowest value with zero false merges over 44 labelled pairs, keeping 67% reuse (§4). The collections and the embedding call exist; the search itself is task 3.4, still open and now first in W5. | `src/topics/`, `src/vectors/collections.ts` | ★★★ |
...
| Test count and pass rate | CI output | Locally: **120 passed + 1 xfailed** (pipeline), **23 passed** (serving). **In CI: 4 runs, 4 failures** — see P34. `pipeline` went green from run 3 once the editable-install fix landed; `serving` failed `npm test` on every run because its CI job had no database. Fixed in `ci.yml`; **green in CI is not yet observed** |
...
- [ ] **Watch CI go green for the first time.** It has run on every push — 4 runs, **4 failures** — and was believed never to have run at all (P34). `serving`'s job had no Postgres for tests that require one; that is now fixed in `ci.yml`, but **a green run has not yet been observed**, and until one is, the fix is a hypothesis.
