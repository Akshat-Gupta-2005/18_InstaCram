...
│   │   └── tests/test_health.py       placeholder test
...
| Pipeline | `ruff` clean; 1/1 tests | `cd services/pipeline && ruff check . && pytest` |
...
- [ ] Decide scraper robots.txt / terms-of-service handling.
...
- [ ] Keep `ollama serve` running with `qwen3:8b` pulled — the stack's model calls depend on the host, not on a cloud key.
...
- [ ] **Provide a contact URL for the scraper** (`SCRAPER_CONTACT`, kept in `.env`, never committed). Wikimedia answers clients that don't identify themselves with `HTTP 403` and "please respect our robot policy"; the same request from an identifying client returns 200. A GitHub repo URL was chosen as the form; the literal value is still needed before the scraper spike can finish.
- [ ] Test a second, non-Wikipedia source before W4 relies on one. A search for "HashMap Java Collections" matched the article *Hash table* — usable grounding, but generic rather than the Java class, so encyclopedia-level sources may not match the granularity these topics need.
