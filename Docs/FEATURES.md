...
| 4 | `services/serving/src/topics/resolve.ts` · `src/vectors/topicIndex.ts` | **built, not yet wired** | `embed(name + description)` is searched against the Topic-Name Vector DB at threshold `0.955`, **and** Postgres is read for same-name topics whose vector is still owed — under an advisory lock on the name, so simultaneous requests cannot both create it (P35). **The branch point.** Tested against real Postgres; not yet called from a feed request. |
| 5a | `services/serving/src/topics/resolve.ts` | **built, not yet wired** | **MATCH** → write one `Field_Topic` row. Pipeline never runs for a `ready` topic. A matched `empty` topic is **re-queued as the same row** — not treated as a miss that creates a duplicate; a candidate proposal is its sanctioned retry event (invariant 6, P16). |
| 5b | `services/serving/src/topics/resolve.ts` | **built, not yet wired** | **NO MATCH** → create the topic as `pending` with its outbox row, in one transaction. The pending row **is** the pipeline's job (DECISIONS 2026-09-16). **The worker that claims it does not exist yet.** |
