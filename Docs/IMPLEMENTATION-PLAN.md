...
| 5.7 | Not started | Adjacent-field LLM fallback when overlap is thin |
...
1. **W5 — cache-then-generate wiring, in progress.** 3.4 and 5.1–5.6 are done, including the acceptance test for the whole design (see the progress table in §9). **Next: 5.7** (adjacent-field fallback when overlap is thin), then the W5 measurements — cache hit rate on a clean database, and time-to-first-card on a hit versus a miss.
