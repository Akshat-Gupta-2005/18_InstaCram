...
1. **W5 — cache-then-generate wiring, in progress.** 3.4 and 5.1–5.5 are done, including the acceptance test for the whole design (see the progress table in §9). **Next: 5.6**, `POST /v1/fields/{id}/expand` — the "more topics" action, which re-runs candidates with existing names excluded and re-queues the field's empty topics; the job table already has `kind = 'more'`. Then 5.7 (adjacent-field fallback), and the W5 measurements — cache hit rate on a clean database, and time-to-first-card on a hit versus a miss.
